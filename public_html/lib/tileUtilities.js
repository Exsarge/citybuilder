...
var TileUtilities = (function () {
  ...
  _createClass(TileUtilities, [{
        ...
        case "center":
          ...
          var point = {
            center: {
              //x: sprite.centerX,
              //y: sprite.centerY
              x: sprite.cartX + sprite.x + sprite.width / 2,
              y: sprite.cartY + sprite.y + sprite.height / 2
            }
          };
          sprite.collisionPoints = point;
          collision.hit = Object.keys(sprite.collisionPoints).some(checkPoints);
    ...
    */
    ...
    value: function makeIsoPointer(pointer, world) {
      Object.defineProperties(pointer, {
        ...
        cartX: {
          get: function get() {
            var x = (2 * this.y + this.x) - (2 * world.y + world.x) / 2;
            return x;
          },
        ...
        cartY: {
          ...
          get: function get() {
            var y = (2 * this.y - this.x) - (2 * world.y - world.x) / 2;
            return y;
          },
    ...
    */
    ...
    key: "isoRectangle",
    value: function isoRectangle(width, height, lineWeight, color, helper) {
      ...
      rectangle.lineStyle(lineWeight, color);
      if (helper) rectangle.beginFill(color);
      rectangle.moveTo(0, 0);
      rectangle.lineTo(width, halfHeight);
      rectangle.lineTo(0, height);
      rectangle.lineTo(-width, halfHeight);
      rectangle.lineTo(0, 0);
      if (helper) rectangle.endFill();
      // draw a helper rectangle
      /*if (helper) {
          var w = rectangle.width;
          var h = rectangle.height;
          rectangle.moveTo(-width, 0);
          ...
          tiledLayer.objects.forEach(function (object) {
                ...
                if (object.gid !== 0) {
                  (function () {
                    ...
                    //Use the above values to create the sprite's image from
                    //the tileset image
                    texture = _this5.frame("assets/L_SS_AllAge_CupBonus1.png", 0, 0, object.width, object.height);
                    ...
                    //G. Add isometric properties to the sprite
                    _this5.addIsoProperties(tileSprite, object.x, object.y, world.cartTilewidth, world.cartTileheight);
                    ...
                    //Make a record of the sprite's `gid` on the tileset.
                    //This will also be useful for collision detection later
                    tileSprite.gid = object.gid;
                    object.sprite = tileSprite;
                    //Add the sprite to the current layer group
                    layerGroup.addChild(tileSprite);
                  })();
                }
